...
class MerchSys_Registration_Field extends MerchSysStore_Common_Base {
	...
	public function __construct($view, $item = null) {
		...
		if (isset($item['title']))$item['title'] = __($item['title'], MerchSysStore_Settings::PLUGIN_NAME);
		if (isset($item['placeholder'])) $item['placeholder'] = __($item['placeholder'], MerchSysStore_Settings::PLUGIN_NAME);
		if (isset($item['label'])) $item['label'] = __($item['label'], MerchSysStore_Settings::PLUGIN_NAME);
		if (isset($item['link'])) {
			$link_var_name = $item['link'].'_URL';
			if (isset(MerchSysStore_Public::$$link_var_name)) $item['link'] = MerchSysStore_Public::$$link_var_name;
			else $item['link'] = '';
		} 
		else {
			$item['link'] = false;
